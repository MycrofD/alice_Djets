...
#include "RooUnfoldBinByBin.h"*/
    ...
    const int Rpar = 3; //-----! jet R parameter for your studies (the one that you use in your ananlysis)
    const double fRpar = 0.3;
    //======  change these numbers based on how many D mesons have been analysized in data and MC ======
    const int ND = 2;
    const int NDMC = 2;

    const double sigma_in = 2.09; // inelastic x-section in bars
    const double nEvScale = 1.017; //-----! scaling factor from the number of selected events to the number of events needed for the nomrmalization, taken from the normalization counter
...
TString OUTDIRECTORY="/home/basia/Work/alice/analysis/pPb_run2/DzeroR03_BaseCuts";
