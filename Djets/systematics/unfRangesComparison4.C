...
,	"350350/unfolding_SVD_5"
,	"350350/unfolding_SVD_6"
,	"350350/unfolding_SVD_7"
//,	"550350/unfolding_Bayes_3"
,	"550350/unfolding_Bayes_4"
//,	"550350/unfolding_Bayes_5"
//,	"550350/unfolding_SVD_5"
,	"550350/unfolding_SVD_6"
//,	"550350/unfolding_SVD_7"
//,	"550550/unfolding_SVD_5"
//,	"550550/unfolding_SVD_6"
//,	"550550/unfolding_SVD_7"
}

TString desc[nFiles] = {
  "meas: 3-50, true: 3-50, Bayes reg=4 (Def)"
...
void unfRangesComparison4(int reg=4,  TString inDirBase = "/home/jackbauer/Work/alice/analysis/pp5TeV/D0jet/results/DzeroR03_pPbCuts", TString inName = "systematics/unfolding/diff_Bayes_v_SVD_std", bool isChain = 0,  int measmin=3, int measmax=50, int truemin=5, int truemax=50)
{
        ...
        //    return;
          ...
          cspec->SaveAs(Form("%s/UnfoldingRangesComparison.png",out.Data()));
            ...
            for(int i=0; i<nFiles-1; i++){
                ...
                hratio[i]->GetYaxis()->SetRangeUser(0.84,1.14);
                hratio[i]->GetYaxis()->SetTitle(Form("ratio to (#it{%s})",desc[0].Data()));
...
/**/
            ...
            hmean->GetYaxis()->SetRangeUser(0.90,1.08);
...
////
            TH1F *hmeansys = (TH1F*)hmean->Clone("hmeansys");
            TCanvas *cspecMeanSys = new TCanvas("cspecMeanSys","cspecMeanSys",800,400);
            hmeansys->Draw("hist");
for (int i=0; i<hmeansys->GetNbinsX(); i++){
if (hmeansys->GetBinContent(i+1)-1 < 0) value=1-hmeansys->GetBinContent(i+1);
else value=hmeansys->GetBinContent(i+1)-1;
hmeansys->SetBinContent(i+1,value*100);
            hmeansys->GetYaxis()->SetRangeUser(0.0,10);
//                    double pt = specReb[i]->GetBinCenter(j);
//                    int bin = spec[i]->GetXaxis()->FindBin(pt);
//                    double value = spec[i]->GetBinContent(bin);
//                    double error = spec[i]->GetBinError(bin);
//                    specReb[i]->SetBinContent(j,value);
//                    specReb[i]->SetBinError(j,error);
cout << hmeansys->GetBinContent(i+1) << endl;
//cout << hmeansys->GetBin(i+1) << endl;

}

            cspecMeanSys->SaveAs(Form("%s/UnfoldingRangesComparison_meanSys.pdf",out.Data()));
            cspecMeanSys->SaveAs(Form("%s/UnfoldingRangesComparison_meanSys.png",out.Data()));
////
            outFile->cd();
            cspec->Write();
            cspec2->Write();
...
//  hmean->GetYaxis()->SetRangeUser(0.95,1.1);
  ...
  for(int i=0; i<nJetBins2; i++){
      ...
      for (int j=0; j<nFiles-1; j++){
          mean[i] = mean[i]+ ( hratio[j]->GetBinContent(
							hratio[j]->FindBin( 
									(ptJetbins2[i]+ptJetbins2[i+1])/2. 
									)
							) 
				);
...
void getAvg(const int nFiles, TH1F **hrat, TH1F *hmean)
{
  hmean->GetYaxis()->SetTitle("mean");
//  hmean->GetYaxis()->SetRangeUser(0.95,1.1);
  hmean->SetMarkerStyle(20);
  ...
  for(int i=0; i<nJetBins2; i++){
      mean[i] = 0;
      for (int j=0; j<nFiles-1; j++){
          mean[i] = mean[i]+ ( hrat[j]->GetBinContent(
							hrat[j]->FindBin( 
									(ptJetbins2[i]+ptJetbins2[i+1])/2. 
									)
							) 
