...
void yieldSys(TString inName, TString inDirBase, int measmin, int measmax, int truemin, int truemax)
{
            ...
            for (int i=0; i<nFiles; i++){
                dirName[i] = inDirBase;
                dirName[i] += "/";
                dirName[i] += "Default_249_doFDSys";
                dirName[i] += "/";
                dirName[i] += "unfolding_Bayes_4";
                dirName[i] += inDir[i];

            }

            TFile *outFile = new TFile(Form("%s/bfeeddownSys.root",out.Data()),"RECREATE");
...
cout<<dirName[0].Data()<<endl;
	    TCanvas *cspec = new TCanvas("cspec","cspec",800,800);
          ...
          cspec->SaveAs(Form("%s/bfeeddown.png",out.Data()));
            ...
            for(int i=0; i<nFiles-1; i++){
                ...
                hratio[i]->GetYaxis()->SetRangeUser(0.80,1.25);
            ...
            cspec2->SaveAs(Form("%s/bfeeddown_ratio.pdf",out.Data()));
            cspec2->SaveAs(Form("%s/bfeeddown_ratio.png",out.Data()));


            //-------------- uncertanties
            TH1D *hsys = (TH1D*)*spec[0]->Clone("hsys");
            hsys->GetYaxis()->SetTitle("FD sys. unc [%]");
            hsys->SetLineColor(kViolet+1);
            hsys->SetLineStyle(2);
            hsys->SetLineWidth(2);
            hsys->GetYaxis()->SetRangeUser(4,16);
		...
		if (unc2<0)unc2 = -unc2;
            ...
            cspecUnc->SaveAs(Form("%s/bfeeddown_sys.pdf",out.Data()));
            cspecUnc->SaveAs(Form("%s/bfeeddown_sys.png",out.Data()));
