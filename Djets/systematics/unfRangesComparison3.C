...
void unfRangesComparison3(int reg=4,  TString inDirBase = "/home/jackbauer/Work/alice/analysis/pp5TeV/D0jet/results/DzeroR03_pPbCuts", TString inName = "systematics/unfolding/diff_Bayes_v_SVD_avg", bool isChain = 0,  int measmin=3, int measmax=50, int truemin=5, int truemax=50)
{
...
void compareRanges(TString inName, TString inDirBase, int measmin, int measmax, int truemin, int truemax)
{
        ...
        //    return;
          ...
          //  }
            ...
            TH1F *spec[nFiles];
            TH1F *specReb[nFilesComp];
            TH1F *specpre[nFilesComp-1];
...
//return;
////getAvg(nFiles,hratio,hmean,hsys);
//getAvg(nFiles,spec,specpre);
//
//            for(int i=1; i<nFilesComp; i++) {
//                specReb[i] = new TH1F(Form("specReb%d",i),"specReb",ptbinsN,ptbinsA);
//                for(int j=1;j<specReb[i]->GetNbinsX()+1;j++){
//                    double pt = specReb[i]->GetBinCenter(j);
//                    int bin = specpre[i-1]->GetXaxis()->FindBin(pt);
//                    double value = specpre[i-1]->GetBinContent(bin);
//                    double error = specpre[i-1]->GetBinError(bin);
//                    specReb[i]->SetBinContent(j,value);
//                    specReb[i]->SetBinError(j,error);
//                    }
//
...
/* this is where averaging SVD ends */
            ...
            for(int i=0; i<nFilesComp-1; i++){
                ...
                hratio[i]->GetYaxis()->SetRangeUser(0.85,1.14);
            ...
            getRMS(nFilesComp,hratio,hmean,hsys);
