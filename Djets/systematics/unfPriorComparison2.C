...
void unfPriorComparison2(int reg=4,  TString inDirBase = "/home/jackbauer/Work/alice/analysis/pp5TeV/D0jet/results/DzeroR03_pPbCuts", TString inName = "systematics/unfolding/diff_priors", bool isChain = 0,  int measmin=3, int measmax=50, int truemin=5, int truemax=50)
{
...
void compareRanges(TString inName, TString inDirBase, int measmin, int measmax, int truemin, int truemax)
{
            ...
            for (int i=0; i<nFiles; i++){
                dirName[i] = inDirBase;
                dirName[i] += "/";
                dirName[i] += "Default_249_350350_prior/unfolding_Bayes_4_priorType";
                dirName[i] += inDir[i];
        ...
        //    return;
          ...
          //  }
            ...
            TFile *outFile = new TFile(Form("%s/UnfoldingPriorsComparison.root",out.Data()),"RECREATE");
          ...
          cspec->SaveAs(Form("%s/UnfoldingPriorsComparison.pdf",out.Data()));
          cspec->SaveAs(Form("%s/UnfoldingPriorsComparison.png",out.Data()));
            ...
            cspec2->SaveAs(Form("%s/UnfoldingPriorsComparison_ratio.pdf",out.Data()));
            cspec2->SaveAs(Form("%s/UnfoldingPriorsComparison_ratio.png",out.Data()));
            ...
            cspecRMS->SaveAs(Form("%s/UnfoldingPriorsComparison_rms.pdf",out.Data()));
            cspecRMS->SaveAs(Form("%s/UnfoldingPriorsComparison_rms.png",out.Data()));

            hmean->GetYaxis()->SetRangeUser(0.96,1.04);
            hmean->SetLineColor(kMagenta+1);
            TCanvas *cspecMean = new TCanvas("cspecMean","cspecMean",800,400);
            hmean->Draw("hist");
            line->Draw("same");

            cspecMean->SaveAs(Form("%s/UnfoldingPriorsComparison_mean.pdf",out.Data()));
            cspecMean->SaveAs(Form("%s/UnfoldingPriorsComparison_mean.png",out.Data()));
