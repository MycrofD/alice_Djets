...
void feedRpPb(TString prod="feedR", TString outdir = "rr"){

const int nJetBins=7;
double ptJetbins[nJetBins+1]={5,6,8,10,14,20,30,50};

double ratiomin=0.5, ratiomax=1.8;
double comin=1.0, comax=200000.0;
const int SE = 1; const int DC = 9; //
TString simName[DC]={"central","F05R1","F05R05","F1R2","F1R05","F2R1","F2R2","mb5","mb45"};
...
for (int j=0; j<SE;j++){        //j=0-> Sideband, j=1-> EffScaled
        for (int i=0; i<DC; i++){
                rootfile=Form("/home/jackbauer/ALICE_HeavyFlavour/work/Djets/alice_Djets/Djets/systematics/rr/JetPtSpectra_%s.root",simName[i].Data());
...
//	hratio[i]->GetYaxis()->SetRangeUser(ratiomin,ratiomax);
	...
	pad1->cd();
	hhs[0][i]->GetYaxis()->SetTitle("R_{pPb}");
...
//	hhs[0][i]->GetYaxis()->SetRangeUser(comin,comax);
	...
	hhs[0][i]->GetYaxis()->SetRangeUser(0.5,1.6);
	if (i==0) hhs[0][i]->Draw();
	...
	hratio[i]->GetYaxis()->SetRangeUser(0.9,1.1);
	if (i){
	hratio[i]->Draw("same");
...
for (int i=0; i<DC; i++){
	...
	hratioC[i]->GetYaxis()->SetRangeUser(0.94,1.06);
	if (i){
	hratioC[i]->Draw("same");
	}
}

TCanvas* cFD = new TCanvas("cFD","cFD",800,600);
TH1D *hsysup = new TH1D("hsysup","FD syst",nJetBins,ptJetbins);
TH1D *hsysdo = new TH1D("hsysdo","FD syst",nJetBins,ptJetbins);

for(int j=1;j<hratioC[0]->GetNbinsX()+1;j++){
		double comin = 1;
		double comax = 1;
	for (int i=0; i<DC; i++){
	       	double content = hratioC[i]->GetBinContent(j);
//cout<<content<<endl;
		if (content > comax){
			comax = content;
		}
...
cout<<comax<<"---"<<1-comin<<"---"<<endl;
hsysup->SetBinContent(j,comax);
hsysdo->SetBinContent(j,comin);
}
hsysup->GetYaxis()->SetRangeUser(0.9,1.1);
hsysup->Draw();
hsysdo->Draw("same");
