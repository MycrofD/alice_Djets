...
if len(sys.argv)==1:
    ...
    print("   === e.g.: python iterations2.py 02 0")
    exit()
...
#----------
plt.figure()
plt.hlines(SimpleVarRatio(hist[1],hist[0]),fptbinsZlo,fptbinsZhi,label='Bayes Iter = 4',colors='red')
plt.hlines(SimpleVarRatio(hist[2],hist[0]),fptbinsZlo,fptbinsZhi,label='Bayes Iter = 6',colors='blue')
...
plt.figure()
plt.hlines(FinalRMS(hist,hist[0]),fptbinsZlo,fptbinsZhi,label='',colors='red')
for a,b in zip(fptbinsZA,FinalRMS(hist,hist[0])):
    plt.text(a,b, '%.2f'%(b))
#plt.legend(loc='upper right')
#plt.plot((0.4,1.0),(1,1),'k.-.')
plt.xlabel('$z_{||}^{ch}$')
