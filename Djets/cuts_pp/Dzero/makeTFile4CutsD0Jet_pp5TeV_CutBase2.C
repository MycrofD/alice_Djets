...
/***************************************************************************/
/***************************************************************************/
// Cuts that give a good performance close to p-Pb case, worse at 20-30 jet pT but better at higher jet pT
...
//____________________________________________________
  ...
  //  RDHFD0toKpitoKpi->SetCutOnzVertexSPD(2);
...
// Cuts for D0 cuts
  ...
  // PILE UP REJECTION
  if(isOnData) {
    RDHFD0toKpi->SetOptPileup(AliRDHFCuts::kRejectPileupEvent);  	   //per DATI (spegni per MC)
    RDHFD0toKpi->ConfigurePileupCuts(5,0.8);  //-> Overloaded by mult dep pileup (F.Prino) //per DATI (spegni per MC)
    //RDHFD0toKpi->SetUseMultDepPileupCut(kTRUE);  	   //NON USATO PIU! //per DATI (spegni per MC)
  }
  ...
  RDHFD0toKpi->SetSelectCandTrackSPDFirst(kTRUE, 4); //Cristina 2016...

  //Trigger selection
  RDHFD0toKpi->SetTriggerClass("");
  if(isOnData) RDHFD0toKpi->SetTriggerMask(AliVEvent::kINT7);
  else RDHFD0toKpi->SetTriggerMask(AliVEvent::kINT7); //A&E said to prefer kINT7 to kMB also in pPb MC
*/
...
RDHFD0toKpi->SetUseCentrality(AliRDHFCuts::kCentOff); //kCentOff,kCentV0M,kCentTRK,kCentTKL,kCentCL1,kCentInvalid
  ...
  TFile* fout;
  if(isOnData) fout=new TFile("D0toKpiCuts_pp5TeV_Base2.root","recreate");   //set this!!
  else fout=new TFile("D0toKpiCuts_pp5TeV_Base2.root","recreate");   //set this!!
