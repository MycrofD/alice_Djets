...
void drawResz()
{
   ...
   for(int i=0;i<2; i++){
       ...
       sparse->GetAxis(6)->SetRangeUser(5,7);
       ...
       sparse->GetAxis(5)->SetRangeUser(0.6,0.8);
       if(i==0) {
       ...
       sparse->GetAxis(5)->SetRangeUser(0.8,2);
       if(i==0) {
         ...
         hbin3->Add(dynamic_cast<TH1D*>(sparse->Projection(11)));
            ...
            hbin1->GetXaxis()->SetTitle("#Delta_{#it{z}}");
   ...
   // TLegend *leg = new TLegend(0.6,0.7,0.9,0.9);
    ...
    leg->AddEntry(hbin1,"0.4 < #it{z}_{||}^{ch,part} < 0.6 GeV/#it{c}","p");
    leg->AddEntry(hbin2,"0.6 < #it{z}_{||}^{ch,part} < 0.8 GeV/#it{c}","p");
    leg->AddEntry(hbin3,"0.8 < #it{z}_{||}^{ch,part} < 1.0 GeV/#it{c}","p");
    ...
    TPaveText *pt = new TPaveText(0.15,0.6,0.5,0.83,"NB NDC");
    pt->SetBorderSize(0);
    pt->SetFillStyle(0);
    pt->SetTextAlign(13);
    pt->SetTextFont(42);
    ...
    text0 = pt0->AddText("ALICE, pp, #sqrt{#it{s}} = 5.02 TeV");
    TText *text = new TText;
    pt->SetBorderSize(0);
    //text = pt->AddText("ALICE, pp, #sqrt{#it{s}} = 5.02 TeV"); //uncomment
    ...
    text = pt->AddText("5 < #it{p}_{T, ch jet}^{part} < 7 GeV/#it{c}");
