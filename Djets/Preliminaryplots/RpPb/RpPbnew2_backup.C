...
TString desc[nFiles] = {
  "pp #sqrt{s} = 5.02 TeV (scaled by A)",
...
double        sysUncErr_ppSq[ptbinsN] = {0.0069,0.003825,0.005425,0.004825,0.010425,0.013825,0.0198};//old RpPb with feed down separated
//double        sysUncErr_ppSq[ptbinsN] = {0.007341,0.004266,0.005866,0.005266,0.010866,0.014266,0.020241};//, new = this +0.000441
//double        sysUncErr_ppSq[ptbinsN] = {0.00734116,0.00426616,0.00586616,0.00526616,0.01086616,0.01426616,0.02024116};//,BR: new = this +0.00000016
...
void RpPbnew2(TString outName = "$HOME/Work/alice/analysis/RpPb/ratio")
{
	...
	sysUncErr_pp[i]= TMath::Sqrt(sysUncErr_ppSq[i]);
            ...
            for(int i=0; i<nFiles; i++) {
                ...
                for(int j=0; j<specReb[i]->GetNbinsX(); j++){
                  ...
                  ptvalun[j] = (ptbinsA[j+1]-ptbinsA[j]) / 9.;
            ...
            TPaveText *pt = new TPaveText(0.145,0.7,0.525,0.9,"NB NDC");
            pt->SetBorderSize(0);
            pt->SetFillStyle(0);
            pt->SetTextAlign(13);
            pt->SetTextFont(43);
...
//cspec2->SetLogx();
//  cspec2->SetLeftMargin(0.12);
//  cspec2->SetBottomMargin(0.11);

  cspec2->SetLeftMargin(0.13);
  cspec2->SetRightMargin(0.07);
  cspec2->SetTopMargin(0.09);
  cspec2->SetBottomMargin(0.13);
  ...
  cspec2->SetTicky(1);
                ...
                hratio->GetYaxis()->SetTitle("#it{R}_{pPb}");
...
double        sysGlobe[1]={1.0};
double        ptvalGx[1]={ptvalun[0]/3};
double        ptvalGy[1]={0.0425440947724};//3.7%,2.1%->0.037^2+0.021^2=0.001369+0.000441=0.00181
double        ptGlo[1]={1.0};

//global uncertainty
                TGraphAsymmErrors *graSysGlobal  = new TGraphAsymmErrors(1,ptGlo,sysGlobe,ptvalGx,ptvalGx,ptvalGy,ptvalGy);
//------------------
...
//                TGraphAsymmErrors *graSys  = new TGraphAsymmErrors(ptbinsN,ptval,sysUnc,ptvalunc,ptvalunc,sysUncErr_do,sysUncErr_up);
                ...
                TGraphAsymmErrors *graSys2  = new TGraphAsymmErrors(ptbinsN,ptval,sysUnc,ptvalun,ptvalun,sysUncErr_do,sysUncErr_up);
                ...
                graSys2->SetFillColor(kWhite);//colors2[2]);
                //graSys->SetFillColor(colors2[5]);
                //graSys->SetFillColor(colors2[5]);
                graSys->SetLineColor(colors2[4]);
                graSys2->SetLineColor(colors2[4]);
                //graSys->SetLineSize(1);
                TH1F *Graph_central_syst_unc = new TH1F("Graph_central_syst_unc","",100,4.8,53);
                Graph_central_syst_unc->SetMinimum(0);
                Graph_central_syst_unc->SetMaximum(2.4);
		...
		Graph_central_syst_unc->GetXaxis()->SetTitleOffset(1.3);
		Graph_central_syst_unc->GetYaxis()->SetTitleOffset(1.0);
                ...
                Graph_central_syst_unc->GetYaxis()->SetTitle("#it{R}_{pPb}");
...
//               graSys->Draw("2A");
               ...
               graSys->Draw("5A");
graSysGlobal->SetFillColor(colors2[4]);
graSysGlobal->SetFillStyle(1001);
graSysGlobal->Draw("e2same");
graSysGlobal->SetLineColor(colors2[4]);
                hratio->Draw("same");
                pt->Draw();

            TLine *line = new TLine(ptbinsA[0],1,ptbinsA[ptbinsN]+3,1);
            line->SetLineStyle(2);
            line->SetLineWidth(0);
            line->Draw("same");


TFile *ofile = new TFile("../RAA_RpPb/RpPb_new.root" ,"RECREATE");
//TFile *ofile = new TFile("RpPb.root" ,"RECREATE");
graSys2->Write("D0jetRpPb_Systematics");
graSysGlobal->Write("D0jetRpPb_GlobalUnc");
hratio->Write("D0jetRpPb");
ofile->Close();
