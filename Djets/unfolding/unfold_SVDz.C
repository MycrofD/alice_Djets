...
const int regSVD = 5,  // default reg. parameter for the bayes unfolding
...
datafile+="/plots/JetPtSpectrum_FDsub.root";

detRMfile+=Form("_Jet%d_%d_Dpt%d_%d.root",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin],(int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]);

bkgRMfile+=Form("_Jet%d_%d_Dpt%d_%d.root",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin],(int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]);

gSystem->Load(Form("%s",roounfoldpwd.Data()));
gStyle->SetOptStat(0000); //Mean and RMS shown
gSystem->Exec(Form("mkdir  %s",outDir.Data()));
gSystem->Exec(Form("mkdir  %s/SVD",outDir.Data()));
outDir+=Form("/SVD/Jetbin_%d_%d",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin]);
gSystem->Exec(Form("mkdir  %s",outDir.Data()));
...
LoadDetectorMatrix(detRMfile.Data(),"hZ2d","hZGen","hZRec",0);
	...
	MatrixDeltaReb = Rebin2D("MatrixDeltaReb", fMatrixDeltaPt, fptbinsZMeasN, fptbinsZMeasA, fptbinsZTrueN, fptbinsZTrueA,0);
    ...
    TH1D* hProjYeffRebin=(TH1D*)hProjYeff->Rebin(fptbinsZTrueN, "hProjYeffRebin", fptbinsZTrueA);
    TH1D* fRawRebin=(TH1D*)fRawSpectrum->Rebin(fptbinsZMeasN, "fRawRebin", fptbinsZMeasA);
...
************************************/
		...
		fUnfoldedSVD[ivar]->SetLineColor(colortable[ivar]);
		fUnfoldedSVD[ivar]->SetLineStyle(1);
		fUnfoldedSVD[ivar]->SetMarkerColor(colortable[ivar]);
		fUnfoldedSVD[ivar]->SetMarkerStyle(fMarkers[ivar]);
		fUnfoldedSVD[ivar]->SetLineWidth(2);
		fUnfoldedSVD[ivar]->GetXaxis()->SetTitle("z_{||,ch}");
		...
		fUnfoldedSVD[ivar]->GetYaxis()->SetRangeUser(2000,15000);

		//if(ivar == 0) { fUnfoldedSVD[ivar]->Draw(); }
		if(ivar == 0 || ivar == 8) {continue; }
		else if(ivar == 1) {fUnfoldedSVD[ivar]->Draw(); }
...
*/
if (fptbinsZMeasN==fptbinsZTrueN){
...
TH1D* singVec = tsvdunf->GetSV();
    ...
    cUnfoldedD->SetLogy();
    TLegend* leg2 =  new TLegend(0.15,0.60,0.45,0.85);
...
unfresult->SetMarkerColor(kBlue);
unfresult->SetMarkerSize(0);
bdat->SetMarkerColor(kGreen+2);
fUnfoldedClone->SetMarkerSize(2);

fUnfoldedClone->Scale(1,"width");
fUnfoldedClone->Draw();//unfolded
leg2->AddEntry(fUnfoldedClone,"RooUnfold SVD");
unfresult->Draw("same");//unfolded
leg2->AddEntry(unfresult,"TSVDUnfold");
bdat->Draw("same");//data
leg2->AddEntry(bdat,"Measured (data)");
leg2->Draw("same");
...
/*---- end of `finding D vector' ---*/
    ...
    for(Int_t ivar=0; ivar<NTrials; ivar++){
        ...
        hRatio[ivar]->SetLineColor(colortable[ivar]);
				...
				int counter = 0;
        for(int kk=1; kk<fptbinsZMeasN; kk++){
                double val = fabs(1-hRatio[ivar]->GetBinContent(kk));
                //cout << "==val: " << val << endl;
                if(val<0.1) counter++;
        }
       //if(counter == fptbinsZMeasN-2) cout << "!!!!!! bin found: " << ivar+1 << endl;
//-------------------
        hRatio[ivar]->GetYaxis()->SetRangeUser(0.7,1.3);
        hRatio[ivar]->GetXaxis()->SetRangeUser(fptbinsZMeasA[0],fptbinsZMeasA[fptbinsZMeasN]);
        //hRatio[ivar]->GetXaxis()->SetRangeUser(plotminx,fptbinsZMeasA[fptbinsZMeasN]);
//        hRatio[ivar]->GetXaxis()->SetRangeUser(0.4,fptbinsZMeasA[fptbinsZMeasN]);
...
//		hRatioS->GetXaxis()->SetRangeUser(0.0,1.0);
		//hRatioS->Draw("nostackhist");
		//hRatioS->Draw();
		//hRatioS->GetXaxis()->SetTitle("z_{||, ch}");
		gPad->BuildLegend(0.15,0.65,0.3,0.9,"");
    //legRatio2->Draw("same");
...
//-------------------
    TLine *line = new TLine(fptbinsZMeasA[0],1,fptbinsZMeasA[fptbinsZMeasN],1);
    line->SetLineStyle(2);
    line->SetLineWidth(2);
    line->Draw("same");
...
//    int iter = 0;
    for(Int_t ivar=0; ivar<NTrials; ivar++){
        ...
        hRatioSpectrum[ivar]->GetYaxis()->SetRangeUser(0.5,1.5);
	...
	//	hRatioUnfS->SetMaximum(hRatioUnfS->GetMaximum("nostack"));
	//	hRatioUnfS->Draw("nostackhist");
	//	hRatioUnfS->GetXaxis()->SetTitle("z_{||, ch}");
    ...
    fRawRebin->SetName("fRawRebin");
		...
		hUnfolded_Unc->GetYaxis()->SetRangeUser(0,1);
 ...
 /* ==================
	* Scale with the bin width
	================== */
		 ...
		 TPaveText *pvEn= new TPaveText(0.65,0.80,0.8,0.85,"brNDC");
		 pvEn->SetFillStyle(0);
		 ...
		 double shift = 0.052;
		 TPaveText *pvJet = new TPaveText(0.15,0.65-shift,0.40,0.7-shift,"brNDC");
		 pvJet->SetFillStyle(0);
		 pvJet->SetBorderSize(0);
		 pvJet->SetTextFont(42);
		 pvJet->SetTextSize(0.023);
		 pvJet->SetTextAlign(11);
		 pvJet->AddText(Form("Charged Jets, Anti-#it{k}_{T}, #it{R} = 0.%d",Rpar));

		 shift+=0.07;
		 TPaveText *pvD = new TPaveText(0.15,0.65-shift,0.40,0.7-shift,"brNDC");
		 pvD->SetFillStyle(0);
		 pvD->SetBorderSize(0);
		 pvD->SetTextFont(42);
		 pvD->SetTextSize(0.023);
		 pvD->SetTextAlign(11);
		 ...
		 shift+=0.07;
		 TPaveText *pvEta = new TPaveText(0.15,0.65-shift,0.4,0.7-shift,"brNDC");
		 pvEta->SetFillStyle(0);
		 pvEta->SetBorderSize(0);
		 pvEta->SetTextFont(42);
		 pvEta->SetTextSize(0.023);
		 pvEta->SetTextAlign(11);
		 pvEta->AddText(Form("|#it{#eta}_{jet}| < 0.%d",9-Rpar));

		 shift+=0.07;
		 TPaveText *pv3 = new TPaveText(0.15,0.65-shift,0.4,0.7-shift,"brNDC");
		 pv3->SetFillStyle(0);
		 pv3->SetBorderSize(0);
		 pv3->SetTextFont(42);
		 pv3->SetTextSize(0.023);
		 pv3->SetTextAlign(11);
    ...
    TLegend* ls = new TLegend(0.15,0.75,0.40,0.87);
    ls->SetBorderSize(0);
    ls->AddEntry(fRawRebin,"Measured","p");
    //ls->AddEntry(fUnfoldedSVD[regSVD-1], Form("Unfolded, SVD reg=%d",(int)regSVD,"p"));
    ls->AddEntry(fUnfoldedSVD[regSVD-1], Form("Unfolded, SVD reg=%d",(int)regSVD));
    ...
    TH1F* hratio = (TH1F*)fUnfoldedSVD[regSVD-1]->Rebin(fptbinsZTrueN,"hratio",fptbinsZTrueA);
TH1F* fRawRebinClone;
	if (fptbinsZTrueN != fptbinsZMeasN){
	    fRawRebinClone = new TH1F("fRawRebinClone","Measured hist, True rebinned",fptbinsZTrueN,fptbinsZTrueA);
			int istart = 0;
			while (fptbinsZTrueA[0] != fptbinsZMeasA[istart]){
			istart++;
			}
	...
	else  fRawRebinClone = (TH1F*)fRawRebin->Clone("fRawRebinClone");
...
int LoadRawSpectrum(TString fn, TString sname, int rebin = 0, TString spostfix="") {
	...
	if(rebin)  fRawSpectrum = (TH1D*)spectrum->Rebin(fptbinsZMeasN,"fRawSpectrum",fptbinsZMeasA);
...
TF1* getPriorFunction(int prior, TH1D* spect, int priorType = 0, TH1D* rawspectrum = NULL) {
	...
	double fitlo = fptbinsZMeasA[0]; // fFitPtMin;
	double fithi = fptbinsZMeasA[fptbinsZMeasN]; // fFitPtMax;
	//fithi = 30;
 ...
 TF1*   PriorFunction = new TF1("PriorFunction","[0]* pow(x,-[1]) * exp(-[1]*[2]/x)",fitlo,fithi);
    ...
    if(priorType == 0)  		{ PriorFunction->FixParameter(1,4.6); PriorFunction->FixParameter(2,4);}
		...
		else if(priorType == 8) { PriorFunction->SetParLimits(1,2,8); PriorFunction->SetParLimits(2,2,8); fitlo = fptbinsZMeasA[0]; }
