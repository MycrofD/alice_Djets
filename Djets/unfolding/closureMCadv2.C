...
double fJetptbinsGenA[fJetptbinsGenN+1]={3,5,7,10,15,50,100};
    ...
    int deltaZbin = 0;
    TString jetpttitle[5] = {"4-5","5-7","7-10","10-15","15-50"};
...
//    gSystem->Exec(Form("mkdir  %s/alljetz2D",outDir.Data()));
    ...
    TH2D* hZjetGRebin = new TH2D("hGenRebin","hGenRebin", fptbinsZTrueN, fptbinsZTrueA, fJetptbinsGenN, fJetptbinsGenA);
    hZjetGRebin->Sumw2();
    ...
    for(int i=0; i<NDMC; i++){
        ...
        sparseMC[i]->GetAxis(1)->SetRangeUser(fJetptbinsA[0],fJetptbinsGenA[fJetptbinsGenN]);
...
//        mcdata[i] = (THnSparseD)mcdata00.Clone();
    ...
    */
    ...
    THnSparseD mcdata4("mcdata","mcdata",minDim,bins,xmin,xmax);
    THnSparseD mcdata5("mcdata","mcdata",minDim,bins,xmin,xmax);
    mcdata0.Sumw2();
    mcdata1.Sumw2();
    mcdata2.Sumw2();
    mcdata3.Sumw2();
...
//    THnSparseD mcdataVX = (THnSparseD)mcdata0.Clone();
    ...
    // A-----------------------
    // All mcdata012345 are filled equally above.
    // -------------------------A
...
//    mcdata5.GetAxis(0)->SetRangeUser(0.4,1.02);
    ...
    mcdata4.GetAxis(1)->SetRangeUser(fJetptbinsA[4],fJetptbinsA[5]);
    mcdata5.GetAxis(1)->SetRangeUser(fJetptbinsA[5],fJetptbinsA[6]);
...
//double fJetptbinsGenA[fJetptbinsGenN+1]={3,5,7,10,15,50,100};
    ...
    TH1D* hzRreb[fJetptbinsN];
    TH1D* hzGreb[fJetptbinsN];
    ...
    //==========================================================
    TH2D* hData2D = new TH2D("hFD_zjet", "hFD_zjet", fptbinsZMeasN, fptbinsZMeasA, fJetptbinsN, fJetptbinsA);
...
//
 ...
 //   TH2D *fUnfoldedBayesScale = (TH2D*)hData2D->Clone();
    ...
    TCanvas *cRespon = new TCanvas("response","response",800,600);
...
//    //Blank canvas for countering the saveas statement above that stops the canvas next to it from getting plotted
//    TCanvas *cResponBlank = new TCanvas("blank","blank",800,600);
    ...
    //************************************
    int finalbins = 4; //5 GeV to 50 GeV
    //for (int binjet=1; binjet<= hData2D->GetNbinsY(); binjet++){
    for (int binjet=1; binjet<= finalbins; binjet++){
        UnfProjectX[binjet-1] = (TH1D*)fUnfoldedBayes[regBayes-1]->ProjectionX(Form("UnfProjectX_%d",binjet-1), binjet+deltaJbin, binjet+deltaJbin, "E");
	    //UnfProjectX[binjet-1]->SetName(Form("UnfProjectX_%d",binjet-1));
        hDataProjectX[binjet-1] = (TH1D*)hData2D->ProjectionX(Form("hDataProjectX_%d",binjet-1), binjet+deltaJRbin, binjet+deltaJRbin, "E");
        UnfProjectXScale[binjet-1] = (TH1D*)UnfProjectX[binjet-1]->Clone();
        UnfProjectXScale[binjet-1]->Scale(1,"width");
        UnfProjectXIterPre[binjet-1] = (TH1D*)fUnfoldedBayes[regBayes-2]->ProjectionX(Form("UnfProjectXIterPre_%d",binjet-1), binjet+deltaJbin, binjet+deltaJbin, "E");
        //************************************Unfolding systematics
        UnfProjectXIterPost[binjet-1] = (TH1D*)fUnfoldedBayes[regBayes]->ProjectionX(Form("UnfProjectXIterPost_%d",binjet-1), binjet+deltaJbin, binjet+deltaJbin, "E");
        //************************************
        for(int binz=1;binz<=hData2D->GetNbinsX();binz++){
            fUnfoldedBayesScale->SetBinContent(binz,binjet+deltaJbin,UnfProjectXScale[binjet-1]->GetBinContent(binz+deltaZbin));
            fUnfoldedBayesScale->SetBinError(binz,binjet+deltaJbin,UnfProjectXScale[binjet-1]->GetBinError(binz+deltaZbin));
        }
    }
   ...
   //     hData2Dplot->SetMarkerSize(1);
        //SetColors------
        hUnfP[binjet-1]->GetXaxis()->SetRangeUser(0.4,1.02);
        fGenSpec2Dproj[binjet+1]->GetXaxis()->SetRangeUser(0.4,1.02);
...
//    //Changing ten binned unfolded hUnfP to 6 binned
//    //----------------------------------------------
//    TCanvas* omg = new TCanvas("xx","xx",300,300);
//    TH1D* hhh3 = (TH1D*)fGenSpec2Dproj[0]->Clone();
//    for (int i=0; i<fGenSpec2Dproj[0]->GetNbinsX(); i++){
//        hhh3->SetBinContent(i+1, hUnfP[0]->GetBinContent(hUnfP[0]->FindBin(hhh3->GetBinCenter(i+1))));
//    }
//    hhh3->Divide(fGenSpec2Dproj[0]);
//    hhh3->Draw();
//    //Changing ten binned unfolded hUnfP to 6 binned hUnfPF
//    //-----------------------------------------------------
//    for(int bin=1; bin<fJetptbinsN+1; bin++){
//        hUnfPF[bin-1] = (TH1D*)fGenSpec2Dproj[bin-1]->Clone();
//        for(int zbin=1; zbin<fGenSpec2Dproj[0]->GetNbinsX()+1; zbin++){
//            hUnfPF[bin-1]->SetBinContent(zbin,
//                                         hUnfP[bin-1]->GetBinContent(
//                                             hUnfP[bin-1]->FindBin(hUnfPF[bin-1]->GetBinCenter(zbin))
//                                             )
//                                        );
//        }
//    }
