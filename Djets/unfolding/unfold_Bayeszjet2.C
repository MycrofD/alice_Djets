    ...
    ***************************/
    ...
    if(DptcutTrue){
        for (int z = 0; z < hZjetRecGenD->GetNbins();z++) {
            ...
            if(DR_center<2 || DG_center<2 || jR_center<fJetptbinsA[0])                      {measurement_ok = kFALSE;}
            if((DG_center<2 && jG_center>=2 )||(DR_center<2 && jR_center>=fJetptbinsA[0] )) {measurement_ok = kFALSE;}
            if((DG_center<2 && jG_center>=5 )||(DR_center<2 && jR_center>=5 ))              {measurement_ok = kFALSE;}
            if((DG_center<3 && jG_center>=7 )||(DR_center<3 && jR_center>=7 ))              {measurement_ok = kFALSE;}
            if((DG_center<5 && jG_center>=10 )||(DR_center<5 && jR_center>=10) )            {measurement_ok = kFALSE;}
            if((DR_center>36 || jR_center>50 || zR_center<0.4 ))                            {measurement_ok = kFALSE;}
