...
LoadDetectorMatrix(detRMfile.Data(),"hZ2d","hZGen","hZRec",0);
	...
	MatrixDeltaReb = Rebin2D("MatrixDeltaReb", fMatrixDeltaPt, fptbinsZMeasN, fptbinsZMeasA, fptbinsZFinalN, fptbinsZFinalA,0);
	...
	else fMatrixProd = (TH2D*)fMatrixPP->Clone("fMatrixProd");//this is my hZ2d
    ...
    TH1D* hProjYeffRebin=(TH1D*)hProjYeff->Rebin(fptbinsZFinalN, "hProjYeffRebin", fptbinsZFinalA);
...
************************************/
    ...
    for(Int_t ivar=0; ivar<NTrials; ivar++){
       ...
       //if(counter == fptbinsZMeasN-2) cout << "!!!!!! bin found: " << ivar+1 << endl;
		...
		hRatioS->SetMinimum(0.96);
		//hRatioS->SetMaximum(hRatioS->GetMaximum("nostack"));
		hRatioS->SetMaximum(1.05);
		hRatioS->Draw("nostackhist");
		hRatioS->GetXaxis()->SetTitle("z_{||, ch}");
		gPad->BuildLegend(0.75,0.65,0.9,0.9,"");
    //legRatio2->Draw("same");

    TLine *line = new TLine(fptbinsZFinalA[0],1,fptbinsZMeasA[fptbinsZMeasN],1);
    line->SetLineStyle(2);
    line->SetLineWidth(2);
    line->Draw("same");
 ...
 /* ==================
	* Scale with the bin width
	================== */
		 ...
		 TPaveText *pvEn= new TPaveText(0.7,0.85,0.8,0.85,"brNDC");
		 pvEn->SetFillStyle(0);
		 ...
		 double shift = 0.32;
		 TPaveText *pvJet = new TPaveText(0.6,0.65-shift,0.9,0.7-shift,"brNDC");
		 ...
		 shift+=0.04;
		 TPaveText *pvD = new TPaveText(0.6,0.65-shift,0.9,0.7-shift,"brNDC");
		 pvD->SetFillStyle(0);
		 pvD->SetBorderSize(0);
		 pvD->SetTextFont(42);
		 ...
		 else pvD->AddText("with D^{0} #rightarrow K^{-}#pi^{+} and cc.");

		 shift+=0.04;
		 TPaveText *pvEta = new TPaveText(0.6,0.65-shift,0.9,0.7-shift,"brNDC");
		 ...
		 shift+=0.04;
		 TPaveText *pv3 = new TPaveText(0.6,0.65-shift,0.9,0.7-shift,"brNDC");
		 pv3->SetFillStyle(0);
		 pv3->SetBorderSize(0);
		 pv3->SetTextFont(42);
    ...
    cSpectra->SetLogy();
    //fRawSpectrum ->Draw();
    fUnfoldedBayes[regBayes-1]->GetXaxis()->SetRangeUser(0.4,1.02);
...
//    TLegend* ls = new TLegend(0.6,0.55,0.9,0.65);
    ...
    hUnfolded_Unc->GetXaxis()->SetRangeUser(0.4,1);
    hUnfolded_Unc->Draw();
    pvEn->Draw("same");
    pv3->Draw("same");
...
int plotSlice(TVirtualPad * p, TH2D * hMtxPP, TH2D * hMtxDpt, TH2D * hMtxRe, TH2D * hMtxPro, const double ptmin, const double ptmax) {
	...
	int binno = (fptbinsZFinalA[1]-fptbinsZFinalA[0]);
