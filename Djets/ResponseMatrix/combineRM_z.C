...
void combineRM_z (
bool isPrompt = 0,
TString outDir = "/media/jackbauer/data/z_out/ResponseMatrix",//"combinedMatrix",
TString detRMFile="",
bool useDeltaPt = 0,
...
if(isPrompt){detRMFile=outDir+"/DetMatrix_prompt_";}
else{detRMFile=outDir+"/DetMatrix_nonPrompt_";}
detRMFile += Form("Jet%d_%d_Dpt%d_%d.root",
(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]);
...
if (useDeltaPt) {
  ...
  fMatrixDeltaPt->Sumw2();
	fMatrixDeltaPt->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  fMatrixDeltaPt->GetYaxis()->SetTitle("z_{||}^{gen.} ");
	fMatrixDeltaPt->SetTitle("Bkg.Fluc. Matrix");
}

//LoadDetectorMatrix(detRMFile.Data(),"hPtJet2d","hPtJetGen","hPtJetRec",0);
LoadDetectorMatrix(detRMFile.Data(),"hZ2d","hZGen","hZRec",0);
...
if (!fMeasSpectrum) { Error("Unfold", "No reconstructed spectrum!"); return 0; }
	...
	fMatrixPP->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  fMatrixPP->GetYaxis()->SetTitle("z_{||}^{gen.} ");
  ...
  if (!fMatrixProd) { Error("Unfold", "Error getting product matrix!"); return 0;	}
	...
	fMatrixProd->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  fMatrixProd->GetYaxis()->SetTitle("z_{||}^{gen.} ");
  ...
  TH2D *Matrix = Rebin2D("Matrix", fMatrixProd, fptbinsZMeasN, fptbinsZMeasA, fptbinsZTrueN, fptbinsZTrueA,0);
  Matrix->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  Matrix->GetYaxis()->SetTitle("z_{||}^{gen.} ");
  Matrix->SetTitle("Combined Matrix");

  TH2D *MatrixProb = Rebin2D("MatrixProb", fMatrixProd, fptbinsZMeasN, fptbinsZMeasA, fptbinsZTrueN, fptbinsZTrueA,0);
  MatrixProb->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  MatrixProb->GetYaxis()->SetTitle("z_{||}^{gen.} ");
  MatrixProb->SetTitle("Combined Prob Matrix");
  NormMatrixY(MatrixProb);
  MatrixProb->GetZaxis()->SetRangeUser(0.0001,1);

  hMtxPro->GetXaxis()->SetTitle("z_{||}^{rec.} ");
  hMtxPro->GetYaxis()->SetTitle("z_{||}^{gen.} ");
...
/*  hMtxPP->GetXaxis()->SetTitle("p_{T,ch jet}^{rec.} (GeV/#it{c})");
  ...
  TFile *outMatrix;
  if(isPrompt) outMatrix = new TFile(Form("%s/combineMatrix_Jet%d_%d_Dpt%d_%d.root",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]),"recreate");
  else outMatrix = new TFile(Form("%s/combineMatrixFD_Jet%d_%d_Dpt%d_%d.root",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]),"recreate");
  fMatrixProd->Write();
  Matrix->Write();
  MatrixProb->Write();
  hMtxPro->Write();
  ...
  Matrix->GetXaxis()->SetRangeUser(fptbinsZMeasA[0],fptbinsZMeasA[fptbinsZMeasN]+5);
  Matrix->GetYaxis()->SetRangeUser(3,fptbinsZTrueA[fptbinsZTrueN]);

  MatrixProb->GetXaxis()->SetRangeUser(fptbinsZMeasA[0],fptbinsZMeasA[fptbinsZMeasN]+5);
  MatrixProb->GetYaxis()->SetRangeUser(3,fptbinsZTrueA[fptbinsZTrueN]);
  ...
  pv3->Draw("same");
		...
		if(isPrompt){
         ...
         cMatrixProb->SaveAs(Form("%s/plots/MatricesProb.png",outDir.Data()));
			...
			cMatrixProd->SaveAs(Form("%s/plots/ProdMatrix_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
      			cMatrixProdProb->SaveAs(Form("%s/plots/ProdMatrixProb_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
			cMatrixProdReb->SaveAs(Form("%s/plots/ProdMatrixRebin_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
      			cMatrixProdProbReb->SaveAs(Form("%s/plots/ProdMatrixPropRebin_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
		}
		else {
			if(useDeltaPt) {
        cMatrix->SaveAs(Form("%s/plots/MatricesFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
        cMatrixProb->SaveAs(Form("%s/plots/MatricesProbFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
      }
      cMatrixProd->SaveAs(Form("%s/plots/ProdMatrixFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
      cMatrixProdProb->SaveAs(Form("%s/plots/ProdMatrixProbFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
			cMatrixProdReb->SaveAs(Form("%s/plots/ProdMatrixRebinFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
      cMatrixProdProbReb->SaveAs(Form("%s/plots/ProdMatrixPropRebinFD_Jet%d_%d_Dpt%d_%d.png",outDir.Data(),(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin], (int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]));
		}
...
void plotSlice(TVirtualPad * p, TH2D * hMtxPP, TH2D * hMtxDpt, TH2D * hMtxRe, TH2D * hMtxPro, const double ptmin, const double ptmax) {
	...
	int binno = (fptbinsZTrueA[1]-fptbinsZTrueA[0]);
...
TH2D * Rebin2D(const char* name, TH2D *h, int nx, const double *binx, int ny, const double *biny, bool crop) {
  ...
  hre->Sumw2();
	...
	for(int ix=ixmin; ix<ixmax; ix++) {
		...
		double xhi = (ix!=nx) ? binx[ix+1] : h->GetXaxis()->GetBinCenter(h->GetfptbinsZMeasN()+1)+epsilon;
		for(int iy=iymin; iy<iymax; iy++) {
			double ylo = (iy!=-1) ? binx[iy]   : h->GetYaxis()->GetBinCenter(0)-epsilon;
			double yhi = (iy!=ny) ? binx[iy+1] : h->GetYaxis()->GetBinCenter(h->GetfptbinsZTrueN()+1)+epsilon;
			int k = 0;
			double hits = 0;
			double ersq = 0;
			for(int jx=0; jx<=h->GetfptbinsZMeasN()+1; jx++) {
				if (h->GetXaxis()->GetBinCenter(jx) <  xlo) continue;
				if (h->GetXaxis()->GetBinCenter(jx) >= xhi) continue;
				for(int jy=0; jy<=h->GetfptbinsZTrueN()+1; jy++) {
