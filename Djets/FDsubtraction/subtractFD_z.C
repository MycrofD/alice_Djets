...
TH1D *hFD_up_fold;
TH1D *hFD_down_fold;
//TString roounfoldpwd = "/RooUnfold-1.1.1/libRooUnfold";

TH1* GetInputHist(TString inFile, string histName,TH1 *hh);
TH1* GetInputSimHist(TString inFile, TH1 *hJetPt_B);
TH1* GetUpSys(TH1D **hFD, int nFiles, TH1D *hFD_up);
TH1* GetDownSys(TH1D **hFD, int nFiles, TH1D *hFD_down);
...
TH1* foldB(TString matrixFile, TH1D *hFD, TH1D *folded, TString roounfoldpwd);
...
void subtractFD_z(
//TString dataFile = "/JetPtSpectra_SB_eff.root",
TString dataFile="",
TString dataAnalysisFile = ".root",
TString simDir ="",
TString comMatrixFile = "/ResponseMatrix/combineMatrixFD",
TString outSpectraDir = "/FDsubtraction",
TString listName = "",
bool isDptcut = 1,
...
{
dataFile+=Form("_jetbin_%d_%d",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin]);
dataFile+="/JetPtSpectra_SB_eff.root";
comMatrixFile+=Form("_Jet%d_%d_Dpt%d_%d.root",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin],(int)fptbinsDA[0], (int)fptbinsDA[fptbinsDN]);

    gStyle->SetOptStat(0000);

    TString outPlotDir = outSpectraDir;
    outPlotDir+=Form("/Jetbin_%d_%d",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin]);
...
cout<<"---------sigma_in = (sigma_in*1000)----"<<sigma_in*1000<<"--------------"<<endl;
cout<<"---------dataLum = nEv/(sigma_in*1000)----"<<dataLum<<"--------------"<<endl;
    double simScaling;
    if(fSystem){
...
//return;
    ...
    for (int nr=simNr; nr<fBsimN; nr++){
        TString file = simDir;
        file += Form("/Jetpt%d_%d",(int)fptbinsJetA[(int)zjetbin-1],(int)fptbinsJetA[(int)zjetbin]);
        file += "/Z_";
        file += fRunB[nr];
        if(zfrac) { file += "_Jetpt"; file += (int)fptbinsJetA[(int)zjetbin-1]; file += "_"; file += (int)fptbinsJetA[(int)zjetbin];  }
...
cout<<"-================="<<file<<"==========="<<endl;
        ...
        hFD_binned[nr] = (TH1D*)htmp->Rebin(fptbinsZTrueN,Form("hFD_binned_%d",nr),fptbinsZTrueA);
    ...
    TH1D *hFD_central_binned = (TH1D*)htmp->Rebin(fptbinsZTrueN,"hFD_central_binned",fptbinsZTrueA);
//TH1D *hFD_central_binned = (TH1D*)hFD_central->Clone("hFD_central_binned");
...
//---------- Defining the histograms outside the if conditional
...
void subtractB_afterFolding(TString matrixFile,TH1D *hFD_central_binned,TH1D *hFD_up,TH1D *hFD_down,TH1D *hData_binned,TString outPlotDir,TString outSpectraDir, bool isSys){
    ...
    hFD_central_binned_fold = (TH1D*)foldB(matrixFile, hFD_central_binned,hFD_central_binned_fold,roounfoldpwd.Data());
    ...
    if(isSys) {
    hFD_down_fold = (TH1D*)foldB(matrixFile,hFD_down,hFD_down_fold,roounfoldpwd.Data());
...
//    TH1D *hFD_up_fold;
    ...
    setHistoDetails(hData_binned_sub_down,2,20,0,2,2);
    	for(int j=1; j<=fptbinsZMeasN; j++ ){
    	        hData_binned_sub_down->SetBinError(j,0);
    	}
    }
    ...
    TFile *ofile = new TFile(Form("%s/JetPtSpectrum_FDsub.root",outPlotDir.Data()),"RECREATE");
    hData_binned->Write();
    hFD_central_binned->Write();
    hFD_central_binned_fold->Write();
    hFD_ratio->Write();

//JetPtSpectra_FDsubUn->Write();
    if(isSys){
    ...
    ofile->Close();
     ...
     double dshift = 0.04;
     double tsize = 0.03;
     TPaveText *pvJet = new TPaveText(0.55,0.62-shift,0.9,0.65-shift,"brNDC");
     pvJet->SetFillStyle(0);
     pvJet->SetBorderSize(0);
     pvJet->SetTextFont(42);
     pvJet->SetTextSize(tsize);
     pvJet->SetTextAlign(11);
     pvJet->AddText(Form("Charged Jets, Anti-#it{k}_{T}, #it{R} = 0.%d",Rpar));

     shift+=dshift;
     TPaveText *pvD = new TPaveText(0.55,0.62-shift,0.9,0.65-shift,"brNDC");
     pvD->SetFillStyle(0);
     pvD->SetBorderSize(0);
     pvD->SetTextFont(42);
     pvD->SetTextSize(tsize);
     pvD->SetTextAlign(11);
     ...
     shift+=dshift;
     TPaveText *pvEta = new TPaveText(0.55,0.62-shift,0.9,0.65-shift,"brNDC");
     pvEta->SetFillStyle(0);
     pvEta->SetBorderSize(0);
     pvEta->SetTextFont(42);
     pvEta->SetTextSize(tsize);
     pvEta->SetTextAlign(11);
     pvEta->AddText(Form("|#it{#eta}_{jet}| < 0.%d",9-Rpar));

     shift+=dshift;
     TPaveText *pv3 = new TPaveText(0.55,0.62-shift,0.9,0.65-shift,"brNDC");
     pv3->SetFillStyle(0);
     pv3->SetBorderSize(0);
     pv3->SetTextFont(42);
     pv3->SetTextSize(tsize);
     pv3->SetTextAlign(11);
     //pv3->AddText(Form("%d < p_{T,%s} < %d GeV/#it{c}",(Int_t)fptbinsDA[0],fDmesonS.Data(),(Int_t)fptbinsDA[fptbinsDN]));
     pv3->AddText(Form("%d < p_{ch jet} < %d GeV/#it{c}",(int)fptbinsJetA[(int)zjetbin-1], (int)fptbinsJetA[(int)zjetbin]) );
    ...
    hData_binned->Draw();
    hData_binned->GetYaxis()->SetRangeUser(fdplotmin,dataplotmax);
...
//    pvEta->Draw("same");
    ...
    //TLegend *leg1 = new TLegend(0.50,0.35,0.90,0.45);
    ...
    shift+=0;
    pv3 = new TPaveText(0.65,0.65-shift,0.85,0.7-shift,"brNDC");
    pv3->SetFillStyle(0);
    pv3->SetBorderSize(0);
    pv3->SetTextFont(42);
    ...
    JetPtSpectra_FDsubUn->GetYaxis()->SetRangeUser(0,1.0);
    JetPtSpectra_FDsubUn->GetXaxis()->SetRangeUser(0.4,1.0);
...
void subtractB_beforeFolding(TH1D *hFD_central_binned,TH1D *hFD_up,TH1D *hFD_down,TH1D *hData_binned,TString outPlotDir,TString outSpectraDir, bool isSys){
    ...
    //___________________________________drawing and saving
    TFile *ofile = new TFile(Form("%s/JetPtSpectrum_FDsub_beforeFolding.root",outPlotDir.Data()),"RECREATE");
    hData_binned->Write();
    hFD_central_binned->Write();
    hFD_ratio->Write();
...
TH1* foldB(TString matrixFile, TH1D *hFD, TH1D *folded, TString roounfoldpwd ){
    ...
    folded = new TH1D("folded","folded",fptbinsZMeasN,fptbinsZMeasA);
...
void setHistoDetails(TH1 *hh, Color_t color, Style_t Mstyle, double Msize, Width_t Lwidth, Style_t Lstyle){
   ...
   // hh->SetName(name.c_str());
    ...
    hh->GetXaxis()->SetTitle("#it{z}_{||} = #frac{#vec{p}_{ch jet}.#vec{p}_{D    } }{#vec{p}_{ch jet}.#vec{p}_{ch jet}}");
