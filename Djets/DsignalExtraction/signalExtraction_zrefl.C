...
#include "configDzero_ppz.h"

void signalExtraction_zrefl(
//  TString data = "$HOME/Work/alice/analysis/pp5TeV/D0jet/outMC/AnalysisResults_fast_R03_D0MC_def.root",
  TString data = "/home/jackbauer/Work/alice/analysis/pp5TeV/D0jet/outMC/AnalysisResults_642_pp5TeV_z.root",
...
)
{
  ...
  int fRebinMass = 2;
  double jetmin = (int)fptbinsJetA[(int)zjetbin-1], jetmax = (int)fptbinsJetA[(int)zjetbin];
  ...
  TString zname= Form("R0%d_%d_%d",(int)Rpar,(int)jetmin,(int)jetmax);
    TFile *ofile = new TFile(Form("%s/reflectionTemplates_%s_%s.root",outdir.Data(), postfix ? listName.Data() : "pp",zname.Data() ),"RECREATE");
    for(int i=0; i<fptbinsDN; i++){
      hsig[i]->Write();
      hrefl[i]->Write();
