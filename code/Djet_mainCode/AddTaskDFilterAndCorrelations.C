AliAnalysisTaskFlavourJetCorrelationsTest *AddTaskDFilterAndCorrelations(
  AliAnalysisTaskSEDmesonsFilterCJTest::ECandidateType cand = AliAnalysisTaskSEDmesonsFilterCJTest::kDstartoKpipi,
  ...
  AliAnalysisTaskFlavourJetCorrelationsTest::ECorrelationMethod CorrMethod = AliAnalysisTaskFlavourJetCorrelationsTest::kConstituent
)
{
  ...
  AliAnalysisTaskFlavourJetCorrelationsTest *taskCorr = new AliAnalysisTaskFlavourJetCorrelationsTest(taskCorrName.Data(), analysiscuts, cand);
  ...
  if(!jetArrBkgname.IsNull())
  {
      ...
      if(CorrMethod == AliAnalysisTaskFlavourJetCorrelationsTest::kResponseMatrix)
      {
  ...
  nameContainerFC3 += suffix;
    ...
    if(!candArrName.IsNull())
    {
        ...
        if (!coutputFC2) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsTest", "Could not find input container '%s'!", nameContainerFC2.Data());
        }
    ...
    if(!sbArrName.IsNull())
    {
        ...
        if (!coutputFC3) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsTest", "Could not find input container '%s'!", nameContainerFC3.Data());
        }
