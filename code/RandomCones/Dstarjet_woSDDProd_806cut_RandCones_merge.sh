...
export ARG="Dstarjet_woSDDProd_806cut_RandCones_merge.C(\"$1\",$2)"
root -b -q -x $ARG
RET=$?
if [ "$RET" != "0" ];then
  echo "======== ERROR : Dstarjet_woSDDProd_806cut_RandCones.C finished with NON zero code: $RET ========"
...
echo "======== Dstarjet_woSDDProd_806cut_RandCones_merge.C finished with exit code: $? ========"
echo "############## memory after: ##############"
free -m
