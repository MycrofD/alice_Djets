...
Bool_t AliAnalysisTaskFlavourJetCorrelationsMy::Run()
{
  ...
  if (matchingAODdeltaAODlevel<=0) {
   ...
   fhstat->Fill(1);
    ...
    fParticlesCont = JetCont->GetParticleContainer();
  ...
  /* Int_t ntrarr=ParticlesCont->GetNParticles();
   for(Int_t i=0;i<ntrarr;i++)
   {
    ...
    while ((jet0 = JetCont->GetNextJet()))
    {
        UInt_t rejectionReason = 0;
        Bool_t OKjet = JetCont->AcceptJet(jet0, rejectionReason);
        if(!OKjet) {
            fhstat->Fill(5);
            continue;
        }
        ...
        Double_t JetPtCorr = jet0->Pt() - jet->Area()*JetCont->GetRhoVal(); //background subtraction
        fhstat->Fill(3); //Jet accepted
        fhPhiJet->Fill(jet0->Phi());
        fhEtaJet->Fill(jet0->Eta());
        fhPtJet->Fill(JetPtCorr);
        
        
    }
    
    if(!njets) return kTRUE;
    ...
    Double_t ptcone = 0.;
    JetCont->ResetCurrentID();
    fParticlesCont->ResetCurrentID();
    AliVTrack* track = 0;
    while ((track = static_cast<AliVTrack*>(fParticlesCont->GetNextAcceptParticle()))) {
    ...
    fDeltaPTRho->Fill(JetCont->GetRhoVal(),deltaPt);
    fRandConeEtaPhi->Fill(random_cone->Eta(), random_cone->Phi());
    
    if (leadingJet) { 
         fDeltaPTLeadPt->Fill(leadingJet->Pt(),deltaPt);
    }
    
  /*  fDeltaPT->Fill(ptcone);
    fDeltaPTRho->Fill(ptcone,JetCont->GetRhoVal());
    fBkgPT->Fill(Bkgpt);
    ...
    AliEmcalJet *random_cone = GetRandomCone(JetCont);
    AliEmcalJet *random_cone_excl_lead = 0;

    //if(maxJetIds[0] != -1) leadingJet = static_cast<AliEmcalJet*>(JetCont->At(maxJetIds[0])); 

    if (leadingJet) {
      random_cone_excl_lead = GetRandomConeExclLead(JetCont, leadingJet);
    }
    
    
    Float_t deltaPt = random_cone->Pt() - JetCont->GetRhoVal() * random_cone->Area();
    */
    ...
    if (leadingJet) { 
        Float_t deltaPt_excl_lead = random_cone_excl_lead->Pt() - JetCont->GetRhoVal() * random_cone_excl_lead->Area();
         fDeltaPT_excl_lead->Fill(deltaPt_excl_lead);
         fDeltaPTCent_excl_lead->Fill(lPercentile,deltaPt_excl_lead);
         fDeltaPTRho_excl_lead->Fill(JetCont->GetRhoVal(),deltaPt_excl_lead);
         fRandConeEtaPhi_excl_lead->Fill(random_cone_excl_lead->Eta(), random_cone_excl_lead->Phi());
         fDeltaPTLeadPt->Fill(leadingJet->Pt(),deltaPt);
         fDeltaPTLeadPt_excl_lead->Fill(leadingJet->Pt(),deltaPt_excl_lead);
    }
...
void AliAnalysisTaskFlavourJetCorrelationsMy::CreateResponseMatrix()
{
...
void AliAnalysisTaskFlavourJetCorrelationsMy::FillDJetHistograms(AliEmcalJet* jet, Double_t rho, Bool_t IsBkg, AliAODEvent* aodEvent)
{
    ...
    if(fCandidateType==kD0toKpi)
    {
        ...
        FillHistogramsD0JetCorr(dzero,z,Dmeson->Pt(),JetPtCorr,jet->Phi(),jet->GetNumberOfTracks(),IsBkg,bDInEMCalAcc,bJetInEMCalAcc,aodEvent);
    }
    else if(fCandidateType==kDstartoKpipi)
    {
        AliAODRecoCascadeHF* dstar=(AliAODRecoCascadeHF*)Dmeson;
        FillHistogramsDstarJetCorr(dstar,z,Dmeson->Pt(),JetPtCorr,jet->Phi(),jet->GetNumberOfTracks(),IsBkg,bDInEMCalAcc,bJetInEMCalAcc);
    }
...
Bool_t  AliAnalysisTaskFlavourJetCorrelationsMy::DefineHistoForAnalysis(){
   ...
   fhPtJet2      = new TH2F("hPtJet2",  "Jet Pt distribution; p_{T} (GeV/c)",nbinsptjet,ptjetlims[0],ptjetlims[1],nbinsptjet,ptjetlims[0],ptjetlims[1]);
   fhPtJet2->Sumw2();
   ...
   fDeltaPTLeadPt = new TH2F("fDeltaPTLeadPt","#Delta p_{T} lead jet",500,0,100,600,-50,100);
   fDeltaPTLeadPt->Sumw2();
   fDeltaPTLeadPt->GetXaxis()->SetTitle("p_{T}^{lead} (GeV/c)");
   fDeltaPTLeadPt->GetYaxis()->SetTitle("#Delta p_{T} (GeV/c)");
   fDeltaPTLeadPt_excl_lead = new TH2F("fDeltaPTLeadPt_excl_lead","#Delta p_{T} lead jet, excl. lead",500,0,100,600,-50,100);
   fDeltaPTLeadPt_excl_lead->Sumw2();
   fDeltaPTLeadPt_excl_lead->GetXaxis()->SetTitle("p_{T}^{lead} (GeV/c)");
   ...
   fRandConeEtaPhi_excl_lead->GetYaxis()->SetTitle("jet #phi");
    ...
    if(fUseMCInfo){
        ...
        const Int_t nAxis=6;
        const Int_t nbinsSparse[nAxis]={nbinsSpsz,nbinsSpsptjet,nbinsSpsptD,nbinsSpsmass,63,100};
        const Double_t minSparse[nAxis]={zlims[0],ptjetlims[0],ptDlims[0],fMinMass,0,0};
        const Double_t maxSparse[nAxis]={zlims[1],ptjetlims[1],ptDlims[1],fMaxMass,6.3,100};
        fNAxesBigSparse=nAxis;

        fhsDphiz=new THnSparseF("hsDphiz","Z, p_{T}^{jet}, p_{T}^{D}, mass., Jet phi, #jet tracks", nAxis, nbinsSparse, minSparse, maxSparse);
    }
...
void AliAnalysisTaskFlavourJetCorrelationsMy::FillHistogramsD0JetCorr(AliAODRecoDecayHF* candidate, Double_t z, Double_t ptD, Double_t ptj, Double_t phij, Int_t ntrackj, Bool_t IsBkg, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc, AliAODEvent* aodEvent){
...
void AliAnalysisTaskFlavourJetCorrelationsMy::FillHistogramsDstarJetCorr(AliAODRecoCascadeHF* dstar,  Double_t z, Double_t ptD, Double_t ptj, Double_t phij, Int_t ntrackj, Bool_t IsBkg, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc){
...
void AliAnalysisTaskFlavourJetCorrelationsMy::FillHistogramsMCGenDJetCorr(Double_t z,Double_t ptD,Double_t ptjet, Double_t phij, Int_t ntrackj, Bool_t bDInEMCalAcc, Bool_t bJetInEMCalAcc){
 ...
 */
AliEmcalJet* AliAnalysisTaskFlavourJetCorrelationsMy::GetJetCone(Double_t radius, Double_t eta, Double_t phi)
{
  Double_t pt = 0;
  ...
  //Int_t nClusters = SumParticles<AliClusterContainer, MAX_CONSTITUENTS>(pt, eta, phi, maxD2, fClusterCollArray, clusters);
    ...
    while ((track = static_cast<AliVTrack*>(fParticlesCont->GetNextAcceptParticle()))) {
        ...
        Double_t DeltaPhi = AliEmcalContainer::RelativePhi(phi, track->Phi());
        Double_t DeltaEta = eta - track->Eta();
        Double_t d2 = DeltaEta*DeltaEta + DeltaPhi*DeltaPhi;
 ...
 */
AliEmcalJet* AliAnalysisTaskFlavourJetCorrelationsMy::GetRandomCone(AliJetContainer* jetCont)
{
  
  TRandom3 *ran = new TRandom3(0);
  Double_t eta = ran->Uniform(jetCont->GetMinEta() + jetCont->GetJetRadius(), jetCont->GetMaxEta() - jetCont->GetJetRadius());
  //Double_t eta = fRandom->Uniform(jetCont->GetMinEta() + jetCont->GetJetRadius(), jetCont->GetMaxEta() - jetCont->GetJetRadius());
  //Double_t phi = ran->Uniform(jetCont->GetMinPhi(), jetCont->GetMaxPhi());
  Double_t phi = ran->Uniform(0, 2*TMath::Pi());
 ...
 */
AliEmcalJet* AliAnalysisTaskFlavourJetCorrelationsMy::GetRandomConePerp(AliJetContainer* jetCont, AliEmcalJet* leadJet)
{
  Double_t eta = fRandom->Uniform(jetCont->GetMinEta() + jetCont->GetJetRadius(), jetCont->GetMaxEta() - jetCont->GetJetRadius());
 ...
 * @param leadJet Leading jet
 */
AliEmcalJet* AliAnalysisTaskFlavourJetCorrelationsMy::GetRandomConeExclLead(AliJetContainer* jetCont, AliEmcalJet* leadJet)
{
    TRandom3 *ran = new TRandom3(0);
  Double_t eta = ran->Uniform(jetCont->GetMinEta() + jetCont->GetJetRadius(), jetCont->GetMaxEta() - jetCont->GetJetRadius());
  //Double_t eta = fRandom->Uniform(jetCont->GetMinEta() + jetCont->GetJetRadius(), jetCont->GetMaxEta() - jetCont->GetJetRadius());
 ...
 */
Bool_t AliAnalysisTaskFlavourJetCorrelationsMy::AreJetsOverlapping(AliEmcalJet* jet1, AliEmcalJet* jet2)
{
