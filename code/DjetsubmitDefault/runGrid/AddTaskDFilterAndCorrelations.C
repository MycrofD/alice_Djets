AliAnalysisTaskFlavourJetCorrelationsMy *AddTaskDFilterAndCorrelations(
  ...
  AliAnalysisTaskFlavourJetCorrelationsMy::ECorrelationMethod CorrMethod = AliAnalysisTaskFlavourJetCorrelationsMy::kConstituent
)
{
  ...
  AliAnalysisTaskFlavourJetCorrelationsMy *taskCorr = new AliAnalysisTaskFlavourJetCorrelationsMy(taskCorrName.Data(), analysiscuts, cand);
  ...
  if(!jetArrBkgname.IsNull())
  {
      ...
      if(CorrMethod == AliAnalysisTaskFlavourJetCorrelationsMy::kResponseMatrix)
      {
  ...
  TString nameContainerF2="normCounter"; 
  ...
  nameContainerFC3 += suffix;
    ...
    AliAnalysisDataContainer *coutput3 = mgr->CreateContainer(nameContainerF2,AliNormalizationCounter::Class(), AliAnalysisManager::kOutputContainer,Form("%s:DmesonsForJetCorrelations", AliAnalysisManager::GetCommonFileName()));
    ...
    if(!candArrName.IsNull())
    {
        ...
        if (!coutputFC2) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsMy", "Could not find input container '%s'!", nameContainerFC2.Data());
        }
    ...
    if(!sbArrName.IsNull())
    {
        ...
        if (!coutputFC3) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsMy", "Could not find input container '%s'!", nameContainerFC3.Data());
        }
    ...
    mgr->ConnectOutput(taskCorr, 3, coutput3);
