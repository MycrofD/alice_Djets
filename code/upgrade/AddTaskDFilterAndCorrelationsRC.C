AliAnalysisTaskFlavourJetCorrelationsRC *AddTaskDFilterAndCorrelationsRC(
  ...
  AliAnalysisTaskFlavourJetCorrelationsRC::ECorrelationMethod CorrMethod = AliAnalysisTaskFlavourJetCorrelationsRC::kConstituent
)
{
  ...
  AliAnalysisTaskFlavourJetCorrelationsRC *taskCorr = new AliAnalysisTaskFlavourJetCorrelationsRC(taskCorrName.Data(), analysiscuts, cand);
  ...
  if(!jetArrBkgname.IsNull())
  {
      ...
      if(CorrMethod == AliAnalysisTaskFlavourJetCorrelationsRC::kResponseMatrix)
      {
  ...
  nameContainerFC3 += suffix;
    ...
    if(!candArrName.IsNull())
    {
        ...
        if (!coutputFC2) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsRC", "Could not find input container '%s'!", nameContainerFC2.Data());
        }
    ...
    if(!sbArrName.IsNull())
    {
        ...
        if (!coutputFC3) {
            ::Error("AliAnalysisTaskFlavourJetCorrelationsRC", "Could not find input container '%s'!", nameContainerFC3.Data());
        }
